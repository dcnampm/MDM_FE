...
import { ServiceDto, GetServicesQueryParam } from 'types/service.type';
...
const Service = () => {
  ...
  const [supplies, setSupplies] = useState<ServiceDto[]>([]);
  ...
  const [getServiceQueryParam, setGetServicesQueryParam] = useState<GetServicesQueryParam>({});
  const keywordSearch = useDebounce(getServiceQueryParam.keyword as string, 500);
  ...
  const [updateServiceModalData, setUpdateServiceModalData] = useState<ServiceDto>({});
  const columns: any = [
    {
      ...
      title: 'Tác vụ', key: 'action', show: true, render: (item: ServiceDto) => (<Menu className='flex flex-row items-center'>
        {hasAuthority(Authority.EQUIPMENT_GROUP_UPDATE) && <Menu.Item>
          <Tooltip title='Cập nhật thông tin dịch vụ'>
            <EditFilled
              onClick={() => {
        ...
        {hasAuthority(Authority.EQUIPMENT_DELETE) && <Menu.Item>
          <Tooltip title='Xóa'>
            <Popconfirm
              ...
              onConfirm={() => handleDeleteService(item.id as number)}
              okText='Xóa'
              cancelText='Hủy'
            >
              <DeleteFilled />
  ...
  const onChangeQueryParams = (key: string, value: string | moment.Moment[] | undefined | number | PageableRequest) => {
    ...
    let getServiceQueryParamClone: GetServicesQueryParam = getServiceQueryParam;
    if (key === '') { //if key is empty, it means that the component should be updated
      setComponentShouldUpdate(true);
    }
    if (key === 'keyword') {
      getServiceQueryParamClone = { ...getServiceQueryParam, keyword: value as string };
    }
    if (key === 'pageable') {
      pagebaleClone = { ...pageableRequest, page: (value as PageableRequest).page, size: (value as PageableRequest).size };
    }
    setGetServicesQueryParam(getServiceQueryParamClone);
    setPageableRequest(pagebaleClone);
    const url = createUrlWithQueryString(location.pathname, getServiceQueryParamClone, pagebaleClone);
    setQueryString(url);
    navigate(url);
  };
  ...
  const pagination: PaginationProps = {
    ...
    showTotal: (total: number) => `Tổng cộng: ${total} dịch vụ`,
  ...
  const handleDeleteService = (id: number) => {
    serviceApi.deleteService(id).then(() => {
      toast.success('Dịch vụ được xoá thành công!');
      onChangeQueryParams('', '');
    });
  };


  const searchListOfService = (queryParams: GetServicesQueryParam, pageableRequest: PageableRequest) => {
    serviceApi.getServices(queryParams, pageableRequest).then((res) => {
      if (res.data.success) {
  ...
  useEffect(() => {
    ...
    setGetServicesQueryParam({
      keyword: query.keyword || '',
    });
    setPageableRequest({
      page: Number(query.page) || 0, size: Number(query.size) || 20,
    });
    searchListOfService(getServiceQueryParam, pageableRequest);
    setLoading(false);
  ...
  return (<div>
    ...
    <ModalCreateService
      ...
      hideCreateServiceModal={() => setShowCreateServiceModal(false)}
      callback={() => {
        onChangeQueryParams('', '');
      }} />
    <ModalUpdateService
      showUpdateServiceModal={showUpdateServiceModal}
      hideUpdateServiceModal={() => setShowUpdateServiceModal(false)}
      callback={() => {
        onChangeQueryParams('', '');
      }}
      updateServiceModalData={updateServiceModalData}
    />
  </div>);
};
